#ifndef __APC_BT_TEST_SERVER__
#define __APC_BT_TEST_SERVER__
#include <ros/ros.h>
...
{
public:
  BTTestServer(std::string node_name, std::string actionlib_name);
  virtual ~BTTestServer()
  {
  }
...
protected:
  /*
      Should print goal contents on the screen and return true or false depending on
...
template <class ActionClass, class ActionGoalConstPtr, class ActionFeedback, class ActionResult>
BTTestServer<ActionClass, ActionGoalConstPtr, ActionFeedback, ActionResult>::BTTestServer(std::string node_name,
                                                                                          std::string actionlib_name)
  : action_server_(nh_, actionlib_name, boost::bind(&BTTestServer::executeCB, this, _1), false)
...
template <class ActionClass, class ActionGoalConstPtr, class ActionFeedback, class ActionResult>
void BTTestServer<ActionClass, ActionGoalConstPtr, ActionFeedback, ActionResult>::executeCB(
    const ActionGoalConstPtr &goal)
{
  ...
  ROS_INFO("Press 'p' for preempting this action, 'a' to abort, anything else to succeed");
  std::string input;
  char key;
