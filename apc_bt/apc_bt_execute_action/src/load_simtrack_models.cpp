...
bool SimtrackLoader::fillGoal(apc_objects_detection_action::LookForObjectGoal &goal)
{
  ...
  if (!fillParameter("/apc/task_manager/mission_objects", list) || !fillParameter("/apc/task_manager/target_object", target))
  {
    return false;
  }
  ...
  else
  {
    ...
    goal.globalSearch = true;
    goal.objectslistglobal = list;
...
{
  ros::init(argc, argv, "load_simtrack_models");
  apc::SimtrackLoader action(ros::this_node::getName(), "lookforobject", "load_simtrack_models_baxter");
  ros::spin();
  return 0;
}
