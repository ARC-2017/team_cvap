...
To run the SimTrack node with the APC 2016 features, run:

```
roslaunch simtrack_nodes simtrack_demo1.launch
```

By default SimTrack is turned off, and it has to be enabled by switching to one of the cameras defined. 

#### Parameters

There are two relevant parameter files:
* [detector/tracker parameters] (simtrack_nodes/config/parameters.yaml)
* [cameras defined with relevant topics] (simtrack_nodes/config/apc_2016_camera_topics.yaml). 
 
#### Cameras defined

The cameras supported for APC 2016 are (defined in [here](simtrack_nodes/config/apc_2016_camera_topics.yaml)):
* `kinect_chest`
* `kinect_head`
* `kinect_external`
* `ids_left`
...
The services SimTrack supports are:

* Switch objects : to select (or switch) the objects SimTrack is looking for in the image run: 

```
rosservice call /simtrack/switch_objects "model_names:
 ['object_name_YYY','object_name_ZZZ',...]"
```

where `object_name_***` *must* correspond to a folder entry in `data/apc_2016_mesh_models/object_models` (the APC 2016 object models folder from the `data` repository).

* Switch cameras : to select the camera SimTrack should subscribe to run:

```
rosservice call /simtrack/switch_camera "camera: 'camera_name'"
```

where `camera_name` *must* correspond to one of the cameras defined in [the camera topics file] (simtrack_nodes/config/apc_2016_camera_topics.yaml). If the `camera_name` has not been defined, SimTrack will unsubscribe from all topics.

* Stop tracking : to unsubscribe from all image topics and effectively stop SimTrack detection and tracking run:

```
rosservice call /simtrack/stop_tracking "{}"
```
