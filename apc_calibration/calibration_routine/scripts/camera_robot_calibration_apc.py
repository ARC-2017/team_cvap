...
from simtrack_nodes.srv import SwitchCameraByName
...
class camera_robot_calibration_ros():
    def __init__(self):
        ...
        self.base_frame_name=rospy.get_param('base_frame_name') #'/base'
        self.camera_frame_name=rospy.get_param('camera_frame_name') #'/kinect2_1_rgb_optical_frame'
        self.robot_ee_frame_name=rospy.get_param('robot_ee_frame_name') #'/right_gripper_base'
        self.marker_frame_name=rospy.get_param('marker_frame_name') #'/ros_groovy'
        self.sensor_name = rospy.get_param('sensor_name')
        self.file_path = rospy.get_param('file_path', '/home/apc/.ros/')
        ...
        #wait for simtrack to come up
        rospy.wait_for_service('/simtrack/switch_camera')
        rospy.wait_for_service('/simtrack/switch_objects')
	
        try:
            #switch_camera_service = rospy.ServiceProxy('/simtrack/switch_camera', SwitchCameraByName)
            #resp = switch_camera_service('kinect_chest')
            switch_camera_service = rospy.ServiceProxy('/simtrack/switch_camera', SwitchCameraByName)
            resp = switch_camera_service(self.sensor_name)
            switch_objects_service = rospy.ServiceProxy('/simtrack/switch_objects', SwitchObjects)
            resp = switch_objects_service([self.marker_frame_name[1:]])
